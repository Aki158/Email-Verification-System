...
abstract class ORM
{
    ...
    public function __construct(array $data = [])
    {
        ...
        if ($this->tableName !== get_class($this) || static::$columnTypes === null){
    ...
    public static function getAll(): array
    {
        ...
        if (!$stmt) throw new RuntimeException(sprintf("Failed to find row for %s", static::class));

        $stmt->execute();
        $allRows = [];
        $result = $stmt->get_result();
        while($row = $result->fetch_assoc()){
            $allRows[] = new static($row);
        }
        return $allRows;
    }
    ...
    public function hasOne($classname): ?ORM
    {
        ...
        $stmt = $db->prepare("SELECT * FROM {$tableName} WHERE {$foreignKey} = ?");
        ...
        $result = $stmt->get_result();
        $data = $result->fetch_assoc();
        return $data !== null ? new Head($data) : null;
    }

    public function belongsTo($classname): ?ORM
    {
        $db = DatabaseManager::getMysqliConnection();
        $tableName = $classname;
        $primaryKey = static::$primaryKey;
